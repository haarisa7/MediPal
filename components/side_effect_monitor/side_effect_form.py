...
from data.side_effect_monitor.patient_side_effect import insert_side_effect_report
from data.medication_tracker.patient_medications import get_daily_patient_medications
from data.side_effect_monitor.side_effect import search_all_side_effects, get_side_effect_id_by_name
from data.medication_tracker.medications import get_drug_display_name
from components.shared.search_component import render_search_interface, clear_search_selection
...
def _render_emergency_contacts():
...
def _handle_submit(side_effect_id, patient_id, severity, notes, patient_medication_id):
...
def render_side_effect_report_form(patient_id):
    """Render the side effect reporting form."""
    _render_header()
    ...
    # Get patient_medication_id
    patient_medication_id = None
    if selected_med_display != "Not sure / Multiple medications":
        med_index = med_options.index(selected_med_display) - 1
        patient_medication_id = daily_meds[med_index]['id']
    ...
    severity = st.select_slider(
        ...
        format_func=lambda x: f"{x} - " + {
            ...
            3: "Significant (Affecting daily life)",
            4: "Severe (Very concerning)",
            5: "Emergency (Seek immediate help)"
        }[x],
        value=3
    )
    ...
    with col1:
        if st.button("✅ Submit Report", type="primary", use_container_width=True):
            _handle_submit(side_effect_id, patient_id, severity, notes, patient_medication_id)
